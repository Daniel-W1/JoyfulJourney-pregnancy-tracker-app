...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  LoginBloc({required this.authRepositoryInterface})
      : super(const LoginStateInitial()) {
    on<LoginEvent>((event, emit) async {
      if (event is LoginEventLogin) {
        emit(const LoginStateLoading());
        final result =
            await authRepositoryInterface.login(loginForm: event.loginForm);

        print('${event.loginForm.password}, ${event.loginForm.username}');
        print('LoginBloc here and other: $result');
        result.fold(
